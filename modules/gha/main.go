...
func (p *Pipeline) asWorkflow() Workflow {
	...
	steps = append(steps, p.registryAuthStep()...)
...
func (p *Pipeline) registryAuthStep() []JobStep {
	return []JobStep{
		p.bashStep("registry-auth", map[string]string{
			"REGISTRY_USERNAME": "${{ vars.DOCKERHUB_USERNAME }}",
			"REGISTRY_PASSWORD": "${{ secrets.DOCKERHUB_PASSWORD }}",
			"REGISTRY_SERVER":   "${{ vars.DOCKERHUB_SERVER }}",
		}),
	}
}
